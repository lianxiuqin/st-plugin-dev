...
function createDefaultPanel(reg, deps) {
  ...
  const defaultFetchTransport = {
    ...
    send: async (text, hooks) => {
      ...
      if (!res.ok || !body2?.ok) throw new Error(body2?.message || `HTTP ${res.status}`);
      if (typeof body2.data?.reasoning === "string" && body2.data.reasoning !== "") hooks?.onReasoning?.(body2.data.reasoning);
      return body2.data?.reply ?? "";
    }
  };
  function doSend() {
    if (sending) return;
    ...
    const finishChain = () => {
      ...
      if (st) st.textContent = "\u5DF2\u6536\u8D77 \xB7 \u70B9\u51FB\u5C55\u5F00\u5168\u6587";
    };
    const ensureChainBox = () => {
      ...
      const head2 = h("div", "uchat-chain-head");
      head2.append(h("span", "uchat-chain-caret"));
      head2.append(h("span", "uchat-chain-label", "\u601D\u8003\u8FC7\u7A0B"));
      const state = h("span", "uchat-chain-state", "\u601D\u8003\u4E2D\u2026");
      head2.append(state);
      const body2 = h("div", "uchat-chain-body");
      chain.append(head2, body2);
      head2.addEventListener("click", () => chain.classList.toggle("open"));
    ...
    const ensureDeltaBox = () => {
      if (deltaRow) return deltaRow.querySelector(".uchat-bubble-box");
      deltaRow = h("div", "uchat-row ai");
      const boxEl = h("div", "uchat-bubble-box");
      renderDefaultBubble(boxEl, { id: 0, role: "ai", content: "", createdAt: "" });
      deltaRow.appendChild(boxEl);
    ...
    const sendPromise = Promise.resolve(transport.send(text.trim(), {
      onDelta(delta) {
        if (!delta) return;
        if (chainBox && !reasoningDone) finishChain();
      ...
      onReasoning(chunk) {
        if (!chunk) return;
        sawReasoning = true;
        const box2 = ensureChainBox();
        const body2 = box2.querySelector(".uchat-chain-body");
        body2.textContent = (body2.textContent ?? "") + chunk;
        scrollBottom();
      }
    }));
...
function shouldReloadOnSessionChange(reason) {
...
function createChatPage(reg, deps) {
  ...
  try {
    const out = picked.render({ active: null, reload() {
      void deps.getActive().then(() => {
      });
    } });
    if (isElement(out)) {
      return {
